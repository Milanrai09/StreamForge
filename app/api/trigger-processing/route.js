...
export async function POST(req) {
  try {
    ...
    console.log("📩 Incoming SNS Body:", body);

    // ─────────────────────────────────────────────
    // 1. ✅ Subscription Confirmation (CRITICAL)
    // ─────────────────────────────────────────────
    ...
    if (body.Type === "Notification") {
      ...
      try {
        ...
        console.log("⚠️ Message is not valid JSON:", body.Message);
      ...
      if (!record) {
        console.log("⚠️ No S3 record found");
    ...
    // ─────────────────────────────────────────────
    // 6. ✅ Always return 200 for unknown SNS types
    // ─────────────────────────────────────────────
    console.log("ℹ️ Ignored SNS message type:", body.Type);
    ...
    return Response.json(
      { error: "Failed to process SNS request" },
      { status: 500 }
    );
  }
}