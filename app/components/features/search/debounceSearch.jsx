...
export default function VideoSearch() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/video/search?q=${encodeURIComponent(debouncedQuery)}`, {
  ...
  const selectSuggestion = (video) => {
    const label = video?.title || video?.namespace || "";
    setQuery(label);
    setSuggestions([]);
    setFocused(false);
  ...
  return (
    <div data-search-root className="relative w-full max-w-lg">
      ...
      >
        ...
        {!query && !focused && (
          <kbd className="hidden sm:inline-flex items-center g
          
      ap-1 rounded-md border border-(--border-default)] bg-(--surface-card-muted)] px-1.5 py-0.5 text-[10px] font-medium text-(--text-muted)]">
            ⌘K
          </kbd>
        )}
      </div>
              ...
              <ul role="listbox" className="max-h-72 overflow-y-auto py-1.5">
                {suggestions.map((video, i) => (
                  <li
                    ...
                    onClick={() => selectSuggestion(video)}
                    className={`
